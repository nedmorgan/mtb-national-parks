...
export default class Home extends Component {
  ...
  findParks = e => {
    ...
    const specificStateParks = nationalParks.data.filter(
      park => park.states == state
    )
    this.setState({ parks: specificStateParks })
    let stateName = this.state.states.filter(
      state => state.abbreviation == this.state.selectedState.acronym
    )
  ...
  checkStates = () => {
    ...
    let hasStatePostedAlready = savedStates.some(
      state => state.acronym == this.state.selectedState.acronym
    )
    if (hasStatePostedAlready == true) {
      this.setState({ stateAdded: true })
    }
    let stateInDatabase = savedStates.filter(
      state => state.acronym == this.state.selectedState.acronym
    )
  ...
  addState = e => {
    ...
    axios
      ...
      .then(res => {
        axios.get('/api/v1/states/').then(res => {
          let statePosted = res.data.filter(
            state => state.acronym == this.state.selectedState.acronym
          )
